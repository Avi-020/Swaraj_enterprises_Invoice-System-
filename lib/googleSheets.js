...
export async function syncInvoiceNoWithSheets() {
  ...
  try {
    ...
    // Compare with localStorage — use whichever is higher
    let localSeq = 0;
    try {
      localSeq = JSON.parse(localStorage.getItem("sw_inv_no") || "{}").seq || 0;
    } catch {}

    const finalSeq = Math.max(maxSeq, localSeq);
    const nextSeq = finalSeq + 1; // next invoice = highest used + 1
    ...
    return getCurrentInvoiceNo(); // fallback to localStorage value
  }
}
