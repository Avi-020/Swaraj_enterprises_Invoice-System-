...
export function downloadInvoicePdf(invoice) {
  ...
  sf(false, 6.5);
  tx("Buyer Name", MID + 1, row1Y + 23);
  sf(false, 6.5);
  tx("Cell No.", MID + 1, row1Y + 30);
  ...
  sf(false, 6.5);
  tx("Department", MID + 1, row2Y + 8);
  sf(false, 8);
  tx("Purchase Order No.", MID + 1, row2Y + 25);
  sf(true, 8);
  tx(poNumber || "", MID + 1, row2Y + 13);
  sf(false, 6.5);
  tx("Truck / Tempo No.", MID + 1, row2Y + 27);
  sf(false, 6.5);
  tx("Dispatch through", MID + 1, row2Y + 30);
  sf(false, 7);
  tx(transportDetails.dispatchThrough || "", MID + 1, row2Y + 35);

  sf(false, 6.5);
  tx("Material Given To", MID + rW + 1, row2Y + 13);
  sf(false, 6.5);
  tx("Destination", MID + rW + 1, row2Y + 10);
  sf(false, 7);
  tx(transportDetails.destination || "", MID + rW + 1, row2Y + 16);
