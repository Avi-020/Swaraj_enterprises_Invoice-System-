...
export function downloadInvoicePdf(invoice) {
  ...
  sf(false, 6.5);
  tx("Purchase Order No.", MID + 1, row2Y + 9);
  sf(true, 8);
  tx(poNumber || "", MID + 1, row2Y + 14);
  sf(false, 6.5);
  tx("Truck / Tempo No.", MID + 1, row2Y + 22);
  sf(false, 6.5);
  // tx("Dispatch through", MID + 1, row2Y + 25);
  sf(false, 7);
