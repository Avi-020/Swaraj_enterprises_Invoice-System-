...
function parseWohr(text) {
  ...
  while ((m = codePattern.exec(text)) !== null) {
    ...
    // ── Description extraction — strict boundaries ──
    // Stop the description at the FIRST of: a qty+NOS pattern, a date (DD-MM-YYYY),
    // "Drawing No", or an 8-digit HSN code — whichever comes first in the text.
    // This prevents qty/date from leaking into the product name.
    let name = "";
    const afterCode = window.replace(/^[\w-]+\s*/, ""); // strip leading code itself

    // Find the earliest boundary marker
    const boundaries = [
      afterCode.search(/\d{1,3}(?:,\d{3})*\.\d{2}\s*\n?\s*NOS\b/i), // qty like "300.00 NOS" or "1,500.00\nNOS"
      afterCode.search(/\b\d{2}-\d{2}-\d{4}\b/), // date DD-MM-YYYY
      afterCode.search(/Drawing\s*No/i), // "Drawing No"
      afterCode.search(/\b8\d{7}\b/), // 8-digit HSN starting with 8
    ].filter((idx) => idx >= 0);

    const cutoff = boundaries.length
      ? Math.min(...boundaries)
      : afterCode.length;
    let rawName = afterCode.substring(0, cutoff).trim();

    // Clean up: collapse whitespace, strip trailing punctuation/newlines
    rawName = rawName
      .replace(/\s{2,}/g, " ")
      .replace(/[\n\r]+/g, " ")
      .trim();
    // Strip trailing duplicate "DIN xxx-x" drawing reference if present
    rawName = rawName.replace(/\s+DIN\s+[\d-]+$/i, "").trim();
