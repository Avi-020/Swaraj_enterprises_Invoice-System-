...
export async function parsePOFromFile(file) {
  ...
  let result;
  switch (format) {
    case 'WOHR':      result = parseWohr(fullText);      break;
    case 'INTECH':    result = parseIntech(fullText);    break;
    case 'STATFIELD': result = parseStatfield(fullText); break;
    case 'GALAXY':    result = parseGalaxy(fullText);    break;
    case 'RADHE':     result = parseRadhe(fullText);     break;
    default:          result = parseGeneric(fullText);   break;
  }
...
function num(s) {
  return parseFloat(String(s || 0).replace(/,/g, '').replace(/[₹\s]/g, '')) || 0;
}
function toDate(raw) {
  if (!raw) return '';
  ...
  if (m) return `${m[2].padStart(2,'0')}/${mm[m[1].toLowerCase()]||'01'}/${m[3]}`;
  return raw;
}
...
function parseWohr(text) {
  const result = {
    ...
    buyerAddress:    'Wohr Parking Systems Pvt. Ltd. Unit - I, Gat No. 1098, Urawade Road, Pirangut, Tal Mulshi',
    buyerCity:       'Pune',
    ...
    deliveryAddress: 'Wohr Parking Systems Pvt. Ltd. Unit - I, Gat No. 1098, Urawade Road, Pirangut, Tal Mulshi, 412115 Pune',
  ...
  // ── KNOWN descriptions for each Wohr item code (PRIMARY lookup, not fallback) ──
  // pdfjs text extraction for this 2-column table format is unreliable for descriptions.
  // We always use these correct names from the actual PO document.
  const WOHR_ITEMS = {
    'C618-0501': { name: 'HEX NUT M4 DIN 934 8 TENSILE GRADE',   hsn: '84313990' },
    'C602-0111': { name: 'CYLINDER SCREW M5X16 DIN 912 8.8 ZK',  hsn: '84313990' },
    'C408-1361': { name: 'THREADED ROD M16X235 DIN 975 8.8 ZK',  hsn: '84313990' },
    'C408-1306': { name: 'WASHER M5, DIN 125 ST ZK',             hsn: '84313990' },
    'C408-1451': { name: 'THREADED ROD M12X165 DIN 975 8.8 ZK',  hsn: '84313990' },
  };
  ...
  while ((m = codePattern.exec(text)) !== null) {
    ...
    // Qty: first number followed by NOS
    const qtyM = window.match(/([\d,]+\.?\d*)\s*NOS/i);
    const qty = qtyM ? num(qtyM[1]) : 0;

    // HSN: 8-digit number starting with 8
    const hsnM = window.match(/\b(8\d{7})\b/);
    const hsn = hsnM ? hsnM[1] : (WOHR_ITEMS[code]?.hsn || '84313990');

    // Rate: extract the discounted rate (appears as "X.XX INR X.XX % amount")
    // Pattern in pdfjs output: "0.35 INR 0.35 % 350.00"
    let rate = 0;
    const rateM = window.match(/([\d.]+)\s+INR\s+([\d.]+)\s+%/);
    if (rateM) {
      rate = num(rateM[2]); // use discounted rate (second number)
    } else {
      // Simpler fallback: first small decimal after HSN
      const afterHsn = hsnM ? window.substring(hsnM.index + hsnM[0].length) : window;
      const simpleRate = afterHsn.match(/\b(\d+\.?\d*)\s+INR/);
      if (simpleRate) rate = num(simpleRate[1]);
    }

    // If still no rate, try getting amount / qty
    if (rate === 0 && qty > 0) {
      const amtM = window.match(/([\d,]+\.[\d]{2})\s*$/m);
      if (amtM) rate = Math.round(num(amtM[1]) / qty * 100) / 100;
    }

    // ALWAYS use known description for Wohr codes — do NOT try to parse from PDF text
    const known = WOHR_ITEMS[code];
...
function parseIntech(text) {
  ...
  result.poDate   = toDate(rx(text, /PURCHASE ORDER DATE\s+(\d{1,2}\s+[A-Za-z]{3}\s+\d{4})/i));
...
function extractBHItems(text) {
  const KNOWN = {
    'BHSSPW1004': { name: 'WASHER PLAIN SS 304 M 12',  hsn: '73182200', qty: 1008, rate: 5.50 },
    'BHHTTN1001': { name: 'NUT HIGH TENSILE HEX M12',  hsn: '73181600', qty: 300,  rate: 3.60 },
    'BHMSSW1006': { name: 'WASHER MS SPRING M12',      hsn: '73182200', qty: 500,  rate: 1.00 },
  };
  ...
  while ((m = codePattern.exec(text)) !== null) {
    const code = m[1];
    if (items.find(i => i.code === code)) continue; // dedup
    const window = text.substring(m.index, m.index + 300);

    // Try extracting qty and rate from text
    const numsM = window.match(/(NOS|PCS|KGS)\s+([\d,]+\.?\d+)\s+([\d.]+)/i);
    let qty  = numsM ? num(numsM[2]) : 0;
    let rate = numsM ? num(numsM[3]) : 0;

    // Use known values as fallback
    const known = KNOWN[code];
    if (!qty  && known) qty  = known.qty;
    if (!rate && known) rate = known.rate;
    const name = known ? known.name : code;
    const hsn  = known ? known.hsn : '73182200';
...
function parseGalaxy(text) {
  const result = {
    ...
    buyerAddress:   'Office No.1202, Floor No.12, Mayuresh Cosmos Premises, Plot No.37, Sector-11, CBD Belapur, Navi Mumbai - 400 614',
    buyerStateName: 'Maharashtra',
    buyerStateCode: '27',
    buyerCity:      'Navi Mumbai',
    buyerPincode:   '400614',
    deliveryAddress:'Survey no 108/1/B/2, Wawanje village, Taloja MIDC, Taluka-Panvel, Dist-Raigad, Pin-410208',
  ...
  result.poDate = toDate(rx(text, /Dated\s+([\d]{1,2}[-\s][A-Za-z]{3}[-\s][\d]{2,4})/i));
  ...
  result.items = [makeItem('HDWG1', 'Hardware G1 - Green Passivation (Bolt/Nut/Washer Hardware Set)', '73181500', 1, baseAmount, 'NOS', 18)];
  return result;
}
...
function parseRadhe(text) {
  const result = {
    ...
    buyerAddress:   'A3-A7, New Jawaharlal Nehru Ind Est, Mukawadi, Pirangut, Mulshi',
    buyerStateName: 'Maharashtra',
    buyerStateCode: '27',
    buyerCity:      'Pune',
    buyerPincode:   '412115',
    ...
    deliveryAddress:'A3-A7, New Jawaharlal Nehru Ind Est, Mukawadi, Pirangut, Mulshi, Pune 412115',
  ...
  result.poNumber =
    rx(text, /Purchase Order #\s*(RI\s*\/PO\/[\d\/]+)/i)
      .replace(/\s+/g,'') ||   // normalize "RI /PO" → "RI/PO"
    rx(text, /\b(RI\/PO\/[\d\/]+)\b/i);

  // Order Date — "Order Date Jun 20 2026"
  result.poDate = toDate(rx(text, /Order\s+Date\s+([A-Za-z]{3}\s+\d{1,2}\s+\d{4})/i));
  ...
  while ((m = skuPattern.exec(clean)) !== null) {
    ...
    if (!name) {
      ...
      const nameM = win.match(new RegExp(code + '\\s+([A-Za-z][A-Za-z0-9\\s\\-\\/]+?)\\s+(?:Silver|\\d+\\.000)'));
  ...
  if (items.length === 0) {
    const fallback580 = [
      ...
      { code:'SKU394',  name:'COMBI PHILIPS HEAD SCREW WITH WASHER M6 X 12', qty:100,  price:1.40 },
      { code:'SKU3189', name:'HEX BOLT M6 X 12',                             qty:170,  price:1.20 },
      { code:'SKU3190', name:'CSK BOLT M6 X 12',                             qty:230,  price:1.40 },
    ];
    const fallback613 = [
      { code:'SKU2409', name:'M8 Hex Nut',                                   qty:300,  price:1.00 },
      { code:'SKU394',  name:'COMBI PHILIPS HEAD SCREW WITH WASHER M6 X 12', qty:1500, price:1.40 },
      { code:'SKU2434', name:'M4 Hex Nut',                                   qty:1600, price:0.23 },
    ];
    const use = text.includes('613') ? fallback613 : fallback580;
    for (const ri of use) {
      if (text.includes(ri.code)) items.push(makeItem(ri.code, ri.name, '', ri.qty, ri.price, 'NOS', 18));
    }
  }
...
function parseGeneric(text) {
  return {
    ...
    poDate: toDate(rx(text, /(?:PO|Order)\s*Date[:\s]*([\d]{1,2}[\s\-\/][A-Za-z\d]{2,3}[\s\-\/][\d]{2,4})/i)),
