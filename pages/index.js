...
import {
  SELLER, COMPANIES, PRODUCTS,
  getPriceForCompany, TAX_RATE, numberToWords,
} from '../lib/demoData'
import { parsePOFromFile } from '../lib/poParser'
import InvoicePreview from '../components/InvoicePreview'
import { saveInvoiceToSheet, getCurrentInvoiceNo, incrementInvoiceNo } from '../lib/googleSheets'
...
function CompanyCombobox({ value, onChange }) {
  ...
  function handleManual(name) {
    ...
    if (match) {
      ...
      onChange({
        id: 'manual', name,
        address: '', gstin: '', stateName: 'Maharashtra',
        stateCode: '27', placeOfSupply: 'Maharashtra',
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <input
        value={query}
        onChange={e => { setQuery(e.target.value); setOpen(true); handleManual(e.target.value) }}
        ...
        <div style={{
          ...
          {filtered.map(c => (
            ...
            >
              ...
              <div style={{ fontSize: '11px', color: '#64748B' }}>{c.gstin} · {c.stateName}</div>
            </div>
          ))}
        </div>
      )}
...
function ProductCombobox({ value, onChange, companyId }) {
  ...
  function select(product) {
    const price = companyId ? getPriceForCompany(product.id, companyId) : product.defaultPrice
    onChange({ productId: product.id, name: product.name, hsn: product.hsn, unit: product.unit, rate: price })
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <input
        value={query}
        onChange={e => { setQuery(e.target.value); setOpen(true); handleManualInput(e.target.value) }}
        onFocus={() => { setOpen(true) }}
      ...
      {open && filtered.length > 0 && (
        <div style={{
          position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 100,
          background: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px',
          boxShadow: '0 4px 20px rgba(0,0,0,0.12)', maxHeight: '200px', overflowY: 'auto',
        }}>
          {filtered.map(p => {
            const price = companyId ? getPriceForCompany(p.id, companyId) : p.defaultPrice
            return (
              <div key={p.id} onClick={() => select(p)}
                style={{ padding: '7px 12px', cursor: 'pointer', borderBottom: '1px solid #f1f5f9', fontSize: '12px' }}
                onMouseEnter={e => e.currentTarget.style.background = '#EFF6FF'}
                onMouseLeave={e => e.currentTarget.style.background = '#fff'}
              >
...
// ─── Empty item ───────────────────────────────────────────────────────────────
const emptyItem = () => ({ productId: '', name: '', hsn: '', unit: 'Nos.', quantity: '', rate: '', disc: '' })
...
export default function InvoiceGenerator() {
  ...
  function buildInvoice() {
    ...
    const buyer = company
      ...
      : {
          name: '— Select Company —', address: '', gstin: '',
          stateName: '', stateCode: '', placeOfSupply: '',
          contactPerson: '', contact: '',
        }

    return {
      invoiceNo, invoiceDate, poNumber, poDate,
      buyer, items: validItems,
    ...
    <>
        ...
        <header className="bg-[#0F2A4A] text-white py-4 px-6 shadow-lg">
          <div className="max-w-screen-xl mx-auto flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-300 text-xs">GST Invoice Generator &nbsp;·&nbsp; {SELLER.gstin}</p>
            </div>
        ...
        <div className="max-w-screen-xl mx-auto px-4 py-6 grid xl:grid-cols-[480px_1fr] gap-6 items-start">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
              <h2 className="font-bold text-slate-800 mb-1 text-sm">📄 Extract Data from PO</h2>
              <p className="text-slate-400 text-xs mb-3">Upload a PO PDF to auto-fill company, PO no., date and items.</p>
              <label className="flex items-center justify-center gap-2 border-2 border-dashed border-slate-300 hover:border-blue-400 rounded-lg p-5 cursor-pointer transition-colors bg-slate-50 hover:bg-blue-50">
                <input ref={fileRef} type="file" accept="application/pdf" onChange={handlePo} className="hidden" />
                <span className="text-slate-500 text-sm">
                  {extracting ? '⏳ Extracting...' : poFile ? `📎 ${poFile} — click to change` : '📎 Upload PO PDF'}
                </span>
              </label>
              {poWarnings.length > 0 && (
                <div className="mt-3 bg-amber-50 border border-amber-200 rounded-lg p-3 space-y-1">
                  {poWarnings.map((w, i) => <div key={i} className="text-xs text-amber-700">⚠️ {w}</div>)}
                </div>
              )}
            </div>
            ...
            {/* Invoice details */}
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
              <h2 className="font-bold text-slate-800 mb-4 text-sm">📋 Invoice Details</h2>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {company && company.id !== 'manual' && (
                    <div className="mt-2 text-xs text-slate-500 bg-slate-50 rounded-lg p-2.5 border border-slate-100 space-y-0.5">
                      <div className="font-semibold text-slate-700">{company.address}</div>
                      <div>GSTIN: {company.gstin} · State: {company.stateName}</div>
                    </div>
                  )}
                  {company && company.id === 'manual' && (
                    <div className="mt-2 space-y-2">
                      {[
                        ...
                        <div key={k}>
                          <label className="text-[10px] text-slate-400 font-semibold uppercase">{lbl}</label>
                          <input
                            value={company[k] || ''}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Invoice Date</label>
                    <input value={invoiceDate} onChange={e => setInvoiceDate(e.target.value)}
                      placeholder="DD-MM-YYYY"
                      className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1.5">PO Number</label>
                    <input value={poNumber} onChange={e => setPoNumber(e.target.value)}
                      placeholder="Auto-filled from PO"
                      className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1.5">PO Date</label>
                    <input value={poDate} onChange={e => setPoDate(e.target.value)}
                      placeholder="Auto-filled from PO"
                      className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Dispatch Through</label>
                    <input value={dispatch} onChange={e => setDispatch(e.target.value)}
                      placeholder="BY OLA AUTO"
                      className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Destination</label>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
              ...
              {!company && (
                <div className="mb-3 text-xs bg-blue-50 border border-blue-100 text-blue-600 rounded-lg p-2.5">
                  💡 Select a company first — prices will auto-fill based on their price list.
                </div>
              )}

              <div className="space-y-3">
                {items.map((it, i) => (
                  <div key={i} className="border border-slate-200 rounded-xl p-3 relative bg-slate-50">
                    {items.length > 1 && (
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      <div>
                        <label className="text-[10px] font-semibold text-slate-400 uppercase block mb-1">HSN/SAC</label>
                      ...
                      <div>
                        ...
                        <input type="number" value={it.quantity}
                          ...
                          className="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-xs bg-white" placeholder="0" />
                      </div>
                      <div>
                        <label className="text-[10px] font-semibold text-slate-400 uppercase block mb-1">
                          Rate ₹ {company && it.productId && it.productId !== 'manual' && (
                            <span className="text-green-600" title="Auto-filled from company price list">●</span>
                          )}
                        </label>
                        <input type="number" step="0.01" value={it.rate}
                          onChange={e => updateItem(i, 'rate', e.target.value)}
                          className="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-xs bg-white" placeholder="0.00" />
                      </div>
                      <div>
                        <label className="text-[10px] font-semibold text-slate-400 uppercase block mb-1">Amount ₹</label>
                        <div className="w-full border border-slate-100 rounded-lg px-2 py-1.5 text-xs bg-white font-semibold text-slate-700 min-h-[28px]">
                          {it.quantity && it.rate
                            ? (Number(it.quantity) * Number(it.rate)).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
                            : '—'}
                        </div>
                      </div>
                    </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={() => setShowPreview(true)} disabled={!canAction}
              ...
              <button onClick={handleGenerate} disabled={!canAction || downloading}
            ...
            {!canAction && (
              <p className="text-xs text-center text-slate-400">
                Select a company · Enter invoice no. · Add at least one product with qty and rate
              </p>
            )}
            {saveStatus === 'saved'   && <div className="text-xs bg-green-50 border border-green-200 text-green-700 rounded-lg p-3 text-center">✅ Saved to Google Sheet! Invoice number auto-updated to next number.</div>}
            {saveStatus === 'skipped' && <div className="text-xs bg-amber-50 border border-amber-200 text-amber-700 rounded-lg p-3 text-center">⚠️ PDF downloaded. Configure Google Sheets in lib/googleSheets.js to auto-increment invoice numbers.</div>}
            {saveStatus === 'error'   && <div className="text-xs bg-red-50 border border-red-200 text-red-700 rounded-lg p-3 text-center">❌ Could not save to Sheet. PDF downloaded. Invoice number not incremented.</div>}
          ...
          <div className="lg:sticky lg:top-4">
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-bold text-slate-800 text-sm">🧾 Live Invoice Preview</h2>
                <span className="text-xs text-slate-400">Scaled to fit · A4 actual size</span>
              </div>
              <div style={{ width: '100%', overflowX: 'auto', background: '#f1f5f9', borderRadius: '8px', padding: '12px' }}>
                <div style={{ width: '794px', transformOrigin: 'top left', transform: 'scale(0.62)' }}>
                  <InvoicePreview invoice={invoice} />
                </div>
                <div style={{ height: `${1123 * 0.62}px`, marginTop: `-${1123 * 0.62}px`, pointerEvents: 'none' }} />
              </div>
            </div>
          </div>
        </div>
      ...
      {showPreview && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-start justify-center p-4 overflow-auto">
          <div className="bg-white rounded-xl w-full mt-4 mb-8" style={{ maxWidth: '860px' }}>
            <div className="flex items-center justify-between p-4 border-b border-slate-200 sticky top-0 bg-white z-10 rounded-t-xl">
              <h3 className="font-bold text-slate-800">Invoice Preview — {invoiceNo}</h3>
